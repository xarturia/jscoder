...
function addUser() {
    ...
    //
    // Mostrar objetos de la caja azul
    let bluebox = document.getElementById("bluebox");
    console.dir(bluebox.innerHTML);
    // Mostrar lista de usuarios
    let users = document.getElementsByClassName('liElement');
    for (const usuarios of users) {
        console.log(usuarios.innerHTML);
    }
    // Mostrar parrafos 
    let p = document.getElementsByTagName('p');
    for (const pargph of p) {
        console.dir('Parrafo: \n' + pargph.innerHTML);
    }
    //
    for (let i = 0; i < users.length; i++) {
        console.log(users[i].innerText /*  + "\n" + users[i].parentNode.innerText */ );
    }
    // agregar lista con el usuario logueado
    let usList = document.getElementById("userList");
    let userElement = document.createElement('li');
    userElement.className = 'liElement2';
    userElement.innerText = 'Username: ' + usr.toUpperCase();
    usList.appendChild(userElement);
    // agregar lista con la fecha de logueo
    let dateList = document.getElementById('userList');
    let dateElement = document.createElement('li');
    dateElement.className = 'liElement2';
    let dt = new Date();
    let time = dt.getHours() + ':' + dt.getMinutes() + ':' + dt.getSeconds();
    dateElement.innerText = 'Date: ' + dt.toLocaleDateString('en-US') + ' ' + time;
